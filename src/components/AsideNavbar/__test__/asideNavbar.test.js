...
const categoryId = "5edbd65159792f2e580f5491"

afterEach(cleanup)

it('renders without crashing', ()=>{
    const div = document.createElement('div');
    ReactDOM.render(<AuthContext.Provider value={{ categories:categories}}>
                            <AsideNavbar categoryId={categoryId}/>
...
it('matches snapshot', ()=>{
    const tree = renderer.create(
        <AuthContext.Provider value={{ categories:categories}}>
            <AsideNavbar categotyId={categoryId}/>
        </AuthContext.Provider>
   ).toJSON();
